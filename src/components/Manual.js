...
function Manual() {
  ...
  function changeIndexUp(index) {
    ...
    try {
      set(refDB(database, "Manual/"), newAboutData);
    } catch (error) {
      Swal.fire({
        icon: "error",
  ...
  async function addDataManual(event) {
    ...
    set(refDB(database, `Manual/${index}/`), {
      topic: getTopic,
    ...
    arrImg.map((img) => {
      ...
      const storageRef = refSR(storage, `/image-manual/${timestampImage}`);
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((url) => {
            set(
              refDB(database, `Manual/${index}/images/${timestampImage}`),
              url
            )
              .then(() => {
                set(refDB(database, `Manual/${index}/updateAt`), timestampImage)
                  .then(() => {
                    set(
                      refDB(
                        database,
                        `Manual/${index}/images/${timestampImage}`
                      ),
                      url
                    )
                      .then(() => {
  ...
  return (
    <section className="container">
      <div className="text">แนะนำการใช้งาน</div>
