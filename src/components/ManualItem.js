...
function ManualItem(props) {
  ...
  function deleteImageContent(key) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const desertRef = refSR(storage, `/image-manual/${key}`);
        // Delete the Image
        deleteObject(desertRef)
          .then(() => {
            remove(refDB(database, `Manual/${index}/images/${key}`))
              .then(() => {
                const dbRef = refDB(database);
                get(child(dbRef, "Manual/"))
                  .then((snapshot) => {
                    let allAboutData = {};
                    if (snapshot.exists()) {
  ...
  function addImageContent(event) {
    ...
    const storageRef = refSR(storage, `/image-manual/${timestamp}`);
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((url) => {
          try {
            set(refDB(database, `Manual/${index}/images/${timestamp}`), url)
              .then(() => {
                const dbRef = refDB(database);
                get(child(dbRef, "Manual/"))
                  .then((snapshot) => {
  ...
  function deleteManual(index) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        newData.pop();
        set(refDB(database, "Manual/"), newData)
          .then(() => {
            getAboutData();
            Swal.fire({
        ...
        Object.keys(images).forEach((key) => {
          let desertRef = refSR(storage, `/image-manual/${key}`);
          // Delete the Image
          deleteObject(desertRef)
            .then(() => {})
            .catch((error) => {
  ...
  function onSubmitManual(event) {
    ...
    set(refDB(database, `Manual/${index}/topic`), newTopic)
      .then(() => {
        set(refDB(database, `Manual/${index}/description`), newDescription)
          .then(() => {
            set(refDB(database, `Manual/${index}/updateAt`), timestamp)
              .then(() => {
                Swal.fire({
